...
func maybeShoot(delta,playerPos): #Called EVERY PHYSICS PROCESS from Warehouse-Node
	...
	var linear_velocity = move_and_slide(directionVector * SPEED) #Executes movement
	if (abs(linear_velocity.x) + abs(linear_velocity.y)) < SPEED: #An unintended collision occurred, most likely while chasing the player
		directionVector = standardVectors[randi() % standardVectors.size()]
	if directionChange:
		surroundingWalls = calculate_surrounding_walls()
...
func canSeePlayer(playerPos):
	var angleToPlayer = position.angle_to_point(playerPos)
	if abs(rotation-angleToPlayer) < PI/2: #If the player must turn more than 90° in any direction to see the player, it cannot actually SEE it.
		return false
	var surroundings = get_world_2d().direct_space_state
	var result = surroundings.intersect_ray(position,playerPos, [self], Layer.ENVIRONMENT + Layer.ENEMY + Layer.PLAYER)
